...
async def test_create_transaction(async_client: AsyncClient) -> None:
    transaction_data = {
        "transaction": {
            ...
            "type": "BUY",
            "received_token_ca": "0xabcdef1234567890",
            "received_token_symbol": "TOKEN",
    ...
    assert transaction["received_token_symbol"] == "TOKEN"
...
async def test_get_transaction_by_id(async_client: AsyncClient) -> None:
    ...
    transaction_data = {
        "transaction": {
            "wallet_address": "0x1234567890abcdef",
            "type": "SELL",
            "spent_token_ca": "0xabcdef1234567890",
    ...
    create_response = await async_client.post("/api/transactions", json=transaction_data)
...
async def test_get_transactions_by_wallet_address(async_client: AsyncClient) -> None:
    ...
    transaction_data = {
        "transaction": {
            ...
            "type": "SWAP",
            "received_token_ca": "0x1111111111111111",
            "received_token_symbol": "TOKEN1",
            "received_token_quantity": 10.0,
            "spent_token_ca": "0x2222222222222222",
...
async def test_update_transaction(async_client: AsyncClient) -> None:
    ...
    response = await async_client.put(f"/api/transactions/{transaction_id}", json=update_data)
    assert response.status_code == 200
